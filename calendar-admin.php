...
class bestwebsite_Admin {
	public static function menus(){
		$page = add_options_page('Bestwebsite FullCalendar', 'Bestwebsite FullCalendar', 'manage_options', 'wp-fullcalendar', array('bestwebsite_Admin','admin_options'));
		wp_enqueue_style('wp-fullcalendar', plugins_url('includes/css/admin.css',__FILE__));
	}


	public static function admin_options(){
		if( !empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'bestwebsite_options_save')){
			foreach($_REQUEST as $option_name => $option_value){
				if(substr($option_name, 0, 5) == 'bestwebsite_'){
				    if( $option_name == 'bestwebsite_scripts_limit' ){ $option_value = str_replace(' ', '', $option_value); } //clean up comma seperated emails, no spaces needed
					update_option($option_name, $option_value);
				}
			}
			if( empty($_REQUEST['bestwebsite_post_taxonomies']) ){ update_option('bestwebsite_post_taxonomies', ''); }
			echo '<div class="updated notice"><p>'.__('Settings saved.').'</p></div>';
		}
		?>
		<div class="wrap">
			<h2>Bestwebsite FullCalendar</h2>
			<div id="poststuff" class="metabox-holder has-right-sidebar">
				<div id="side-info-column" class="inner-sidebar">
					<div id="categorydiv" class="postbox ">
						...
						<div class="inside">
							<p>This plugin was developed by <a href="https://bestwebsite.com//">bestwebsite</a> and is now provided free of charge thanks to proceeds from the <a href="http://wp-events-plugin.com/">Events Manager</a> Pro plugin.</p>
					...
					<div id="categorydiv" class="postbox ">
						...
						<div class="inside">
							...
							<p>Please email any translations to wp.plugins@bestwebsite.com and we'll incorporate it into the plugin.</p>
						</div>
					</div>
				</div>
				<div id="post-body">
						...
						<form action="" class="calendar-options" method="post">
							...
							<p>
								<ul class="calendar-post-types">
									<?php 
									$selected_taxonomies = get_option('bestwebsite_post_taxonomies');
									foreach( get_post_types( apply_filters('bestwebsite_get_post_types_args', array('public'=>true ))) as $post_type ){
		 								$checked = get_option('bestwebsite_default_type') == $post_type ? 'checked':'';
		 								$post_data = get_post_type_object($post_type);
										echo "<li><label><input type='radio' class='calendar-post-type' name='bestwebsite_default_type' value='$post_type' $checked />&nbsp;&nbsp;{$post_data->labels->name} (<em>$post_type</em>)</label>";
										do_action('bestwebsite_admin_options_post_type_'.$post_type);
										...
										if( count($post_type_taxonomies) > 0 ){
											...
											foreach( $post_type_taxonomies as $taxonomy_name ){
												...
												echo "<li><label><input type='checkbox' name='bestwebsite_post_taxonomies[$post_type][$taxonomy_name]' value='1' $tax_checked />&nbsp;&nbsp;{$taxonomy->labels->name} (<em>$taxonomy_name</em>)</label></li>";
											}
											echo "</ul>";
											echo "</div>";
										}
				            ...
				            <script type="text/javascript">
				            	jQuery(document).ready(function($){
					            	$('input.calendar-post-type').change(function(){
					            		...
					            		$('input[name=bestwebsite_default_type]:checked').parent().parent().find('div').show();
					            	});
					            });
				            </script>
							...
							<table class='form-table'>
								<?php 
								$available_views = apply_filters('bestwebsite_available_views',array('month'=>'Month','basicWeek'=>'Week (basic)','basicDay'=>'Day (basic)','agendaWeek'=>'Week (agenda)','agendaDay'=>'Day (agenda)'));
								?>
								<tr>
									<th scope="row"><?php _e('Available Views','wpfc'); ?></th>
									<td>
										<?php $bestwebsite_available_views = get_option('bestwebsite_available_views', array('month','basicWeek','basicDay')); ?>
										<?php foreach( $available_views as $view_key => $view_value ): ?>
										<input type="checkbox" name="bestwebsite_available_views[]" value="<?php echo $view_key ?>" <?php if( in_array($view_key, $bestwebsite_available_views) ){ echo 'checked="checked"'; } ?>/> <?php echo $view_value; ?><br />
										<?php endforeach; ?>
										<em><?php _e('Users will be able to select from these views when viewing the calendar.'); ?></em>
									</td>
								</tr>
								<?php
								bestwebsite_options_select( __('Default View','wpfc'), 'bestwebsite_defaultView', $available_views, __('Choose the default view to be displayed when the calendar is first shown.','wpfc') );
								bestwebsite_options_input_text ( __( 'Time Format', 'wpfc' ), 'bestwebsite_timeFormat', sprintf(__('Set the format used for showing the times on the calendar, <a href="%s">see possible combinations</a>. Leave blank for no time display.','wpfc'),'http://arshaw.com/fullcalendar/docs/utilities/formatDate/'), 'h(:mm)t' );
								bestwebsite_options_input_text ( __( 'Events limit', 'wpfc' ), 'bestwebsite_limit', __('Enter the maximum number of events to show per day, which will then be preceded by a link to the calendar day page.','wpfc') );
								bestwebsite_options_input_text ( __( 'View events link', 'wpfc' ), 'bestwebsite_limit_txt', __('When the limit of events is shown for one day, this text will be used for the link to the calendar day page.','wpfc') );
								?>
							</table>
							...
							<table class='form-table'>
								...
								$jquery_themes = apply_filters('bestwebsite_jquery_themes', $jquery_themes);
							    ...
							    <tr class="form-field">
							        ...
							        <td>
							            <select name="bestwebsite_theme_css">
							            	<option><?php _e( 'No Theme','wpfc' ); ?></option>
							            	<optgroup label="<?php _e('Built-In','wpfc'); ?>">
								            	<?php foreach( $jquery_themes as $jquery_theme ): ?>
								            	<option <?php if(get_option('bestwebsite_theme_css') == $jquery_theme) echo 'selected="selected"'; ?>><?php echo esc_html($jquery_theme); ?></option>
								            	<?php endforeach; ?>
							            	</optgroup>
							            	<?php if( !empty($css_custom_files) ): ?>
							            	<optgroup label="<?php _e('Custom','wpfc'); ?>">
							            		<?php foreach( $css_custom_files as $css_custom_file ): ?>
							            			<option <?php if(get_option('bestwebsite_theme_css') == $css_custom_file) echo 'selected="selected"'; ?>><?php echo esc_html($css_custom_file); ?></option>
							            		<?php endforeach; ?>
							            	</optgroup>
							            	<?php endif; ?>
							            </select>
							...
							<table class='form-table'>
							    <?php
								bestwebsite_options_radio_binary ( __( 'Enable event tooltips?', 'wpfc' ), 'bestwebsite_qtips', '' );
								$tip_styles = array();
								foreach( WP_FullCalendar::$tip_styles as $tip_style ){
									$tip_styles[$tip_style] = $tip_style;
								}
								bestwebsite_options_select(__('Tooltip style','wpfc'), 'bestwebsite_qtips_style', $tip_styles, __('You can choose from one of these preset styles for your tooltip.','wpfc'));
								bestwebsite_options_radio_binary ( __( 'Rounded tooltips?', 'wpfc' ), 'bestwebsite_qtips_rounded', __( 'If your chosen tooltip style doesn\'t already do/prevent this, you can add rounded corners using CSS3.','wpfc' ) );
								bestwebsite_options_radio_binary ( __( 'Add shadow to tooltips?', 'wpfc' ), 'bestwebsite_qtips_shadow', __( 'If your chosen tooltip style doesn\'t already do/prevent this, you can add a CSS3 drop-shadow effect to your tooltip.','wpfc' ) );
								$positions_options = array();
								foreach( WP_FullCalendar::$tip_positions as $position ){
									$positions_options[$position] = $position;
								}
								bestwebsite_options_select ( __( 'Tooltip pointer position', 'wpfc' ), 'bestwebsite_qtips_my', $positions_options, __( 'Choose where the pointer will be situated on your tooltip.','wpfc' ) );
								bestwebsite_options_select ( __( 'Tooltip bubble position', 'wpfc' ), 'bestwebsite_qtips_at', $positions_options, __( 'Choose where your tooltip will be situated relative to the event link which triggers the tooltip.','wpfc' ) );
								bestwebsite_options_radio_binary ( __( 'Enable featured image?', 'wpfc' ), 'bestwebsite_qtips_image', __('If your post has a featured image, it will be included as a thumbnail.','wpfc') );
							    ?>
								<tr>
									<td><label><?php  _e('Featured image size','wpfc'); ?></label></td>
									<td>
										<?php _e('Width','wpfc'); ?> : <input name="bestwebsite_qtips_image_w" type="text" style="width:40px;" value="<?php echo get_option('bestwebsite_qtips_image_w'); ?>" /> 
										<?php _e('Height','wpfc'); ?> : <input name="bestwebsite_qtips_image_h" type="text" style="width:40px;" value="<?php echo get_option('bestwebsite_qtips_image_h'); ?>" />
									</td>
								</tr>
							</table>
				            ...
				            <table class="form-table">
								<?php
								bestwebsite_options_input_text( __( 'Load JS and CSS files on', 'dbem' ), 'bestwebsite_scripts_limit', __('Write the page IDs where you will display the FullCalendar on so CSS and JS files are only included on these pages. For multiple pages, use comma-seperated values e.g. 1,2,3. Leaving this blank will load our CSS and JS files on EVERY page, enter -1 for the home page.','wpfc') );
								?>
							</table>
							
							
							<input type="hidden" name="_wpnonce" value="<?php echo wp_create_nonce('bestwebsite_options_save'); ?>" />
...
//check for updates
if( version_compare(bestwebsite_VERSION, get_option('bestwebsite_version',0)) > 0 && current_user_can('activate_plugins') ){
	include('calendar-install.php');
}
//add admin action hook
add_action ( 'admin_menu', array('bestwebsite_Admin', 'menus') );


/*
 * Admin UI Helpers
*/
function bestwebsite_options_input_text($title, $name, $description, $default='') {
	?>
...
function bestwebsite_options_input_password($title, $name, $description) {
	?>
	<tr valign="top" id='<?php echo esc_attr($name);?>_row'>
		<th scope="row"><?php echo esc_html($title); ?></th>
	    <td>
...
function bestwebsite_options_textarea($title, $name, $description) {
	?>
	<tr valign="top" id='<?php echo esc_attr($name);?>_row'>
		<th scope="row"><?php echo esc_html($title); ?></th>
			<td>
...
function bestwebsite_options_radio($name, $options, $title='') {
...
function bestwebsite_options_radio_binary($title, $name, $description, $option_names = '') {
...
function bestwebsite_options_select($title, $name, $list, $description, $default='') {
