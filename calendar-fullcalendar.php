<?php
/*
Plugin Name: Bestwebsite FullCalendar
Version: 1.0
Plugin URI: https://github.com/bestwebsite/bestwebsite-fullcalendar
...
define('bestwebsite_UI_VERSION','1.0'); //jQuery 1.11.x
...
class WP_FullCalendar{
	...
	public static function init() {
		...
		add_action('wp_ajax_bestwebsite_qtip_content', array('WP_FullCalendar','qtip_content') );
		add_action('wp_ajax_nopriv_bestwebsite_qtip_content', array('WP_FullCalendar','qtip_content') );
		//base arguments
		self::$args['type'] = get_option('bestwebsite_default_type','event');
	...
	public static function enqueue_scripts(){
	    ...
	    $bestwebsite_scripts_limit = get_option('bestwebsite_scripts_limit');
	    if( empty($bestwebsite_scripts_limit) || in_array($obj_id, explode(',',$bestwebsite_scripts_limit)) ){
		    //Scripts
		    wp_enqueue_script('wp-fullcalendar', plugins_url('includes/js/main.js',__FILE__), array('jquery', 'jquery-ui-core','jquery-ui-widget','jquery-ui-position', 'jquery-ui-selectmenu'), bestwebsite_VERSION); //jQuery will load as dependency
		    WP_FullCalendar::localize_script();
		    //Styles
		    wp_enqueue_style('wp-fullcalendar', plugins_url('includes/css/main.css',__FILE__), array(), bestwebsite_VERSION);
		    //Load custom style or jQuery UI Theme
		    $bestwebsite_theme = get_option('bestwebsite_theme_css');
		    if( preg_match('/\.css$/', $bestwebsite_theme) ){
		        ...
		        if( file_exists(get_stylesheet_directory()."/plugins/wp-fullcalendar/".$bestwebsite_theme) ){
		            $bestwebsite_theme_css = get_stylesheet_directory_uri()."/plugins/wp-fullcalendar/".$bestwebsite_theme;
            	    wp_deregister_style('jquery-ui-css'); 
            	    wp_enqueue_style('jquery-ui-css', $bestwebsite_theme_css, array('wp-fullcalendar'), bestwebsite_VERSION);
		        }
		    }elseif( !empty($bestwebsite_theme) ){
        	    ...
        	    if( $jquery_ui_version != bestwebsite_UI_VERSION ){
        		    ...
        		    if( !empty($jquery_ui_css_versions) ){
        		        ...
        		        $jquery_ui_css_uri = plugins_url(trailingslashit($jquery_ui_css_folder).$bestwebsite_theme."/jquery-ui$min.css",__FILE__);
        		        $bestwebsite_theme_css = plugins_url(trailingslashit($jquery_ui_css_folder).$bestwebsite_theme.'/theme.css',__FILE__);
        		    }
        	    }
        	    if( empty($bestwebsite_theme_css) ){
    		        //use default theme
    		        $jquery_ui_css_uri = plugins_url('/includes/css/jquery-ui/'.$bestwebsite_theme."/jquery-ui$min.css",__FILE__);
    		        $bestwebsite_theme_css = plugins_url('/includes/css/jquery-ui/'.$bestwebsite_theme.'/theme.css',__FILE__);
    		    }
            	if( !empty($bestwebsite_theme_css) ){   
            	    wp_deregister_style('jquery-ui-css'); 
            	    wp_enqueue_style('jquery-ui-css', $jquery_ui_css_uri, array('wp-fullcalendar'), bestwebsite_VERSION);
            	    wp_enqueue_style('jquery-ui-css-theme', $bestwebsite_theme_css, array('wp-fullcalendar'), bestwebsite_VERSION);
            	}
		    }
	    }
	}
	...
	public static function localize_script(){
		...
		$js_vars['bestwebsite_theme'] = get_option('bestwebsite_theme_css') ? true:false;
		$js_vars['bestwebsite_limit'] = get_option('bestwebsite_limit',3);
		$js_vars['bestwebsite_limit_txt'] = get_option('bestwebsite_limit_txt','more ...');
		//FC options
		$js_vars['timeFormat'] = get_option('bestwebsite_timeFormat', 'h(:mm)t');
		$js_vars['defaultView'] = get_option('bestwebsite_defaultView', 'month');
		$js_vars['weekends'] = get_option('bestwebsite_weekends',true) ? 'true':'false';
		$js_vars['header'] = new stdClass();
		$js_vars['header']->right = implode(',', get_option('bestwebsite_available_views', array('month','basicWeek','basicDay')));
		//qtip options
    	$js_vars['bestwebsite_qtips'] = get_option('bestwebsite_qtips',true) == true;
		if( $js_vars['bestwebsite_qtips'] ){
    		$js_vars['bestwebsite_qtips_classes'] = 'ui-tooltip-'. get_option('bestwebsite_qtips_style','light');
    		$js_vars['bestwebsite_qtips_my'] = get_option('bestwebsite_qtips_my','top center');
    		$js_vars['bestwebsite_qtips_at'] = get_option('bestwebsite_qtips_at','bottom center');
    		if( get_option('bestwebsite_qtips_rounded', false) ){
    			$js_vars['bestwebsite_qtips_classes'] .= " ui-tooltip-rounded";
    		}
    		if( get_option('bestwebsite_qtips_shadow', true) ){
    			$js_vars['bestwebsite_qtips_classes'] .= " ui-tooltip-shadow";
    		}
		}
		...
		$calendar_languages = bestwebsite_get_calendar_languages();
		if( array_key_exists($locale_code, $calendar_languages) ){
		    $js_vars['bestwebsite_locale'] = $calendar_languages[$locale_code];
		}elseif( array_key_exists($locale_code_short, $calendar_languages) ){
			$js_vars['bestwebsite_locale'] = $calendar_languages[$locale_code_short];
		}
		$js_vars['bestwebsite_locale']['firstDay'] =  $js_vars['firstDay']; //override firstDay with wp settings
		wp_localize_script('wp-fullcalendar', 'WPFC', apply_filters('bestwebsite_js_vars', $js_vars));
	}
	...
	public static function ajax(){
	    ...
	    //initiate vars
	    $args = apply_filters('bestwebsite_fullcalendar_args', array_merge($_REQUEST, $args));
		$limit = get_option('bestwebsite_limit',3);
	    $items = array();
	    ...
	    function bestwebsite_temp_filter_where( $where = '' ) {
	    	$where .= " AND post_date >= '".date("Y-m-d", $_REQUEST['start'])."' AND post_date < '".date("Y-m-d", $_REQUEST['end'])."'";
	    	return $where;
	    }
	    add_filter( 'posts_where', 'bestwebsite_temp_filter_where' );
		$the_query = new WP_Query( $args );
	    remove_filter( 'posts_where', 'bestwebsite_temp_filter_where' );
	    ...
	    while ( $the_query->have_posts() ) { $the_query->the_post();
	    	...
	    	if( empty($item_date_counts[$post_date]) || $item_date_counts[$post_date] < $limit ){
	    		...
	    		$items[] = apply_filters('bestwebsite_ajax_post', $item, $post);
	    		...
	    		$more_array = array ("title" => get_option('bestwebsite_limit_txt','more ...'), "color" => get_option('bestwebsite_limit_color','#fbbe30'), "start" => $day_ending, 'post_id' => 0, 'allDay' => true);
	    		global $wp_rewrite;
	    		...
	    		$items[] = apply_filters('bestwebsite_ajax_more', $more_array, $post_date);
	    	}
	    }
	    echo json_encode(apply_filters('bestwebsite_ajax', $items));
	...
	public static function qtip_content(){
		...
		if( !empty($_REQUEST['post_id']) ){
	        ...
	        if( get_option('bestwebsite_qtips_image',1) ){
	            $post_image = get_the_post_thumbnail($post->ID, array(get_option('bestwebsite_qtip_image_w',75),get_option('bestwebsite_qtip_image_h',75)));
	            if( !empty($post_image) ){
	                $content = '<div style="float:left; margin:0px 5px 5px 0px;">'.$post_image.'</div>'.$content;
	            }
	        }
	    }
		echo apply_filters('bestwebsite_qtip_content', $content);
		die();
	}
	...
	public static function calendar( $args = array() ){
		...
		self::$args = apply_filters('bestwebsite_fullcalendar_args', self::$args);
		...
		<div id="calendar-calendar-search" style="display:none;">
			<?php
				...
				$bestwebsite_post_taxonomies = get_option('bestwebsite_post_taxonomies');
				$search_taxonomies = !empty($bestwebsite_post_taxonomies[$post_type]) ? array_keys($bestwebsite_post_taxonomies[$post_type]):array();
				...
				do_action('bestwebsite_calendar_search', self::$args);
			?>
		</div>
...
// action links
add_filter('plugin_action_links_'.plugin_basename(__FILE__), 'bestwebsite_settings_link', 10, 1);
function bestwebsite_settings_link($links) {
